...
QMap<QMetaType::Type, VariableType *> VariableType::mTypeBehaviorMap;

VariableType * VariableType::behavior(const QMetaType::Type metaType)
{
    if (mTypeBehaviorMap.contains(metaType))
        return mTypeBehaviorMap.value(metaType);
    ...
    default:                    qFatal("Unhandled QVariant: %i %s", metaType,
...
VariableType::VariableType(QMetaType::Type varType,
    ...
    mVarType(varType),
    mNameParts(partNames.simplified().split(' ', QString::SkipEmptyParts))
{
    mTypeBehaviorMap.insert(varType, this);
}
...
{
    return mNameParts.isEmpty() ? 1 : mNameParts.size();
}

int VariableType::csvParts(void) const
{
    return mNameParts.isEmpty() ? 0 : mNameParts.size();
}

QString VariableType::csvHeader(void) const
{
    return '"' + mNameParts.join("\",\"") + '"';
}
