...
UnitVectorGraph::UnitVectorGraph(void)
{
    TRACEFN()
}

void UnitVectorGraph::configure(const QVariantMap & settings)
{
    mParms.replace(settings);
}

void UnitVectorGraph::clearFormCache()
{
    mFormPixmapCache = QPixmap();
    WEXPECT(mFormPixmapCache.isNull());
}

void UnitVectorGraph::clearGraphCache()
{
    mGraphPixmapCache = QPixmap();
    WEXPECT(mGraphPixmapCache.isNull());
}

QPixmap UnitVectorGraph::formPixmap(void)
{
    TRACEFN()
#if 1
...
QPixmap UnitVectorGraph::graphPixmap(VectorColumnSet * columnSet)
{
    TRACEFN()
    ...
    for (int x = 1; x <= 3; x++)
        if (columnSet->contains(VectorColumnRole::Column(x)))
            mParms.drawColumn(painter, columnSet->value(VectorColumnRole::Column(x)), x);
...
UnitVectorGraph::Parms::Parms(const QVariantMap & settings)
{
    TRACEFN()
    set(settings);
}

void UnitVectorGraph::Parms::set(const QVariantMap & settings)
{
    TRACEFN()
    mSettings = settings;
}

void UnitVectorGraph::Parms::replace(const QString & key,
...
void UnitVectorGraph::Parms::replace(const QVariantMap & settings)
{
    TRACEFN()
...
QVariant UnitVectorGraph::Parms::value(const QString & key,
...
QSize UnitVectorGraph::Parms::pixmapSize() const
{
    TRACEFN()
...
QRect UnitVectorGraph::Parms::graphRect(void) const
{
    TRACEFN()
...
QRect UnitVectorGraph::Parms::borderBoundRect(void) const
{
    TRACEFN()
...
QRect UnitVectorGraph::Parms::borderDrawRect(void) const
{
    TRACEFN()
    int thick = borderThickness();
...
void UnitVectorGraph::Parms::drawGraphBackground(QPainter *painter)
{
...
void UnitVectorGraph::Parms::drawColumn(QPainter * painter,
...
int UnitVectorGraph::Parms::maxCoefs() const
{
    return value("MaxCoefs", 320).toInt();
}

int UnitVectorGraph::Parms::maxVectors() const
{
    return value("MaxVectors", 3).toInt();
}

int UnitVectorGraph::Parms::verticalScale() const
{
    return value("VerticalScale", 100).toInt();
}

int UnitVectorGraph::Parms::interGap() const
{
    return value("InterGap", 1).toInt();
}

int UnitVectorGraph::Parms::intraGap() const
{
    return value("IntraGap", 0).toInt();
}

int UnitVectorGraph::Parms::graphBarThickness() const
{
    return  RoundUp(value("BarThickness", 3).toInt());
}

QColor UnitVectorGraph::Parms::graphBaseBackground() const
{
    return QColor(value("GraphBaseBackgroundColor", "#9CF").toString());
}

int UnitVectorGraph::Parms::marginWidth() const
{
    return value("MarginWidth", 10).toInt();
}

int UnitVectorGraph::Parms::marginHeight() const
{
    return value("MarginHeight", 10).toInt();
}

int UnitVectorGraph::Parms::borderThickness() const
{
    return RoundUp(value("BorderThickness", 3).toInt());
}

QColor UnitVectorGraph::Parms::borderColor() const
{
    return QColor(value("BorderColor", "#006").toString());
}

QBrush UnitVectorGraph::Parms::borderBrush() const
{
    return QBrush(borderColor());
}

QPen UnitVectorGraph::Parms::borderPen() const
{
    return QPen(borderBrush(), borderThickness());
}

QColor UnitVectorGraph::Parms::marginBackgroundColor(void) const
{
    TRACEFN()
...
QPoint UnitVectorGraph::Parms::graphOrigin() const
{
...
int UnitVectorGraph::Parms::graphZeroY() const
{
    QRect drawRect = borderDrawRect();
    int vertScale = verticalScale();
...
QLine UnitVectorGraph::Parms::graphZeroLine() const
{
    QRect drawRect = borderDrawRect();
    int y = graphZeroY();
...
QPen UnitVectorGraph::Parms::graphZeroPen() const
{
    return QPen(borderColor());
}

QLine UnitVectorGraph::Parms::graphLine(const UnitFloat value,
                                        const int coefIndex,
...
QPen UnitVectorGraph::Parms::graphPen(const QColor & color)
{
...
int UnitVectorGraph::Parms::RoundUp(const int value, const bool odd)
{
