...
#define QPROPERTY_DECLARE(TYPE, VAR, NAME, DFLT) \
    ...
    public: \
      ...
      /*TYPE ptr##NAME(void)        { return (VAR *)(& prop##NAME); }*/  \

#define PRIVATEQPROP_DECLARE(TYPE, VAR, NAME, DFLT) \
    private: \
        TYPE prop##NAME = (VAR)(DFLT); \
        void init##NAME(void)      { setProperty(#NAME, QVariant((VAR)(DFLT))); } \
        void set##NAME(TYPE v)     { setProperty(#NAME, QVariant((VAR)(v))); } \
        TYPE get##NAME(void) const { return TYPE(property(#NAME).value<VAR>()); } \
        TYPE ptr##NAME(void)        { return & TYPE(property(#NAME).value<VAR>()); } \
        TYPE ptr##NAME(void)        { return VAR(& prop##NAME); }  \

#define READONLYQPROP_DECLARE(TYPE, VAR, NAME, DFLT) \
    private: \
        TYPE prop##NAME = (VAR)(DFLT); \
        void init##NAME(void)       { setProperty(#NAME, QVariant(VAR(DFLT))); } \
        void set##NAME(TYPE v)      { setProperty(#NAME, QVariant(VAR(v))); } \
...
#define DECLARE_PRIVATEQPROPS(DEF) \
        DEF(QPROPERTY_DECLARE) \

#define DECLARE_READONLYQPROPS(DEF) \
        DEF(QPROPERTY_DECLARE) \
